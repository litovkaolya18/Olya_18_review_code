...
//FIX_ME: неверный синтаксис include - не допустимы пробел после # 
/*
# include <iostream>
# include <windows.h>
# include <vector>
...
//FIX_ME: имя функции конфликтует с std::find и нарушает стиль
void find_selected_items(const vector<vector<int>>& a, int k, int s, const vector<int>& w, double& weight)
{
	if (k == 0 || s == 0) return; 
